#!/usr/bin/env bash
# H32 training run: H29's 288 rows for 5 epochs on physical GPU1, rank 128.
#
# Three capacity points now rise together: rank 16 scored 92, rank 32 scored 94,
# rank 64 scored 96, and the number of cases that flip between neighbouring
# candidates fell from fifteen to ten to eight. The dataset has not changed by a
# byte since H29, so the curriculum was never the binding constraint. H32 asks
# whether a fourth doubling still buys anything or the curve has flattened.
...
OUT="data-private/runs/train/naia-v15-qwen38-27b-gpu1-r128-e5-lr5e5-h32"
...
[ -f "data-private/datasets/naia-v15/manifest.json" ] || { echo "FATAL: H32 dataset manifest missing; build before allocating the GPU" >&2; exit 2; }
...
LOG="$(mktemp -t naia-h32-train-XXXXXX.log)"
...
run_container() {
  podman run --rm --replace \
    ...
    python3 scripts/train_lora.py \
      ...
      --rank 128 --seed 42 --gpu-memory-gib 23 \
...
set +e
{
  echo "=== H32 preflight $(date -Is) ==="
  run_container preflight --preflight-only &&
  echo "=== H32 training $(date -Is) ===" &&
  run_container train &&
  echo "=== H32 done $(date -Is) ==="
} 2>&1 | tee "$LOG"
status="${PIPESTATUS[0]}"
