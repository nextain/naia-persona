#!/usr/bin/env bash
# H30 training run: H29's 288 rows for 5 epochs on physical GPU1, rank 32.
#
# Six experiments changed only the data and the score kept moving sideways: H29
# gained eight benchmark cases over H28 and lost seven, which a sign test cannot
# separate from chance. Re-judging H29's own answers three times moved one case
# out of a hundred, so the ruler is not the cause. The movement is the model.
#
# The likely reason is capacity. A rank-16 adapter has limited room, and the
# curriculum asks it to hold five behaviours at once. Rows added for one axis
# push out what another axis had learned. H30 tests exactly that by holding the
# dataset byte for byte and the step count at 90, and doubling rank alone.
# lora_alpha tracks rank in train_lora.py, so the update scale is unchanged and
# only the room to store the behaviour grows.
set -euo pipefail
...
OUT="data-private/runs/train/naia-v15-qwen38-27b-gpu1-r32-e5-lr5e5-h30"
...
[ -f "data-private/datasets/naia-v15/manifest.json" ] || { echo "FATAL: H30 dataset manifest missing; build before allocating the GPU" >&2; exit 2; }
...
LOG="$(mktemp -t naia-h30-train-XXXXXX.log)"
...
run_container() {
  podman run --rm --replace \
    ...
    python3 scripts/train_lora.py \
      ...
      --rank 32 --seed 42 --gpu-memory-gib 23 \
...
set +e
{
  echo "=== H30 preflight $(date -Is) ==="
  run_container preflight --preflight-only &&
  echo "=== H30 training $(date -Is) ===" &&
  run_container train &&
  echo "=== H30 done $(date -Is) ==="
} 2>&1 | tee "$LOG"
status="${PIPESTATUS[0]}"
