#!/usr/bin/env bash
# H24 evaluation: one model load, all five frozen suites, GPU1 only.
# Identical to the H23 evaluation in every respect except the adapter under test,
...
OUT="data-private/runs/eval/h24"
...
set +e
{
  echo "=== H24 evaluation $(date -Is) ==="
  podman run --rm \
    --name naia-persona-h24-eval \
  ...
  echo "=== H24 evaluation done $(date -Is) ==="
} 2>&1 | tee "$LOG"
status="${PIPESTATUS[0]}"
