#!/usr/bin/env bash
# H27 training run: 277 rows for 5 epochs on physical GPU1.
# Identity is rebuilt as 30 rows with 30 distinct answers, and a format-compliance
# curriculum teaches that an explicit instruction outranks the identity reflex.
# 277 rows at 5 epochs is again 90 optimizer steps.
set -euo pipefail
...
OUT="data-private/runs/train/naia-v15-qwen38-27b-gpu1-r16-e5-lr5e5-h27"
DATA_SHA="398fe8c2778cf5a1f28f2bc84bb6d0947a23ad506a5ea5dbcb2521131dfaad24"
GPU_UUID="GPU-d584beef-b086-bdff-b43c-c31a1b56a611"

cd "$REPO"

[ -f "$DATA" ] || { echo "FATAL: dataset missing: $DATA" >&2; exit 2; }
[ -f "data-private/datasets/naia-v15/manifest.json" ] || { echo "FATAL: H27 dataset manifest missing; build before allocating the GPU" >&2; exit 2; }
...
LOG="$(mktemp -t naia-h27-train-XXXXXX.log)"
...
set +e
{
  echo "=== H27 preflight $(date -Is) ==="
  run_container preflight --preflight-only &&
  echo "=== H27 training $(date -Is) ===" &&
  run_container train &&
  echo "=== H27 done $(date -Is) ==="
} 2>&1 | tee "$LOG"
status="${PIPESTATUS[0]}"
