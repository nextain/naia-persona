#!/usr/bin/env bash
# H28 training run: 287 rows for 5 epochs on physical GPU1.
# Identity keeps 40 distinct answers but every one carries the name anchor, and
# ten rows teach the parent-model distinction explicitly. H27 diluted the name
# out of 20 of 30 answers and the parent's self-identification came back.
set -euo pipefail
...
OUT="data-private/runs/train/naia-v15-qwen38-27b-gpu1-r16-e5-lr5e5-h28"
DATA_SHA="463a8361a3b86caf53d3b978352d39160ed042b3a3ef2032074e81714b076107"
GPU_UUID="GPU-d584beef-b086-bdff-b43c-c31a1b56a611"

cd "$REPO"

[ -f "$DATA" ] || { echo "FATAL: dataset missing: $DATA" >&2; exit 2; }
[ -f "data-private/datasets/naia-v15/manifest.json" ] || { echo "FATAL: H28 dataset manifest missing; build before allocating the GPU" >&2; exit 2; }
...
LOG="$(mktemp -t naia-h28-train-XXXXXX.log)"
...
set +e
{
  echo "=== H28 preflight $(date -Is) ==="
  run_container preflight --preflight-only &&
  echo "=== H28 training $(date -Is) ===" &&
  run_container train &&
  echo "=== H28 done $(date -Is) ==="
} 2>&1 | tee "$LOG"
status="${PIPESTATUS[0]}"
