#!/usr/bin/env bash
# H29 training run: 287 rows for 5 epochs on physical GPU1.
...
OUT="data-private/runs/train/naia-v15-qwen38-27b-gpu1-r16-e5-lr5e5-h29"
...
[ -f "data-private/datasets/naia-v15/manifest.json" ] || { echo "FATAL: H29 dataset manifest missing; build before allocating the GPU" >&2; exit 2; }
...
LOG="$(mktemp -t naia-h29-train-XXXXXX.log)"
...
set +e
{
  echo "=== H29 preflight $(date -Is) ==="
  run_container preflight --preflight-only &&
  echo "=== H29 training $(date -Is) ===" &&
  run_container train &&
  echo "=== H29 done $(date -Is) ==="
} 2>&1 | tee "$LOG"
status="${PIPESTATUS[0]}"
