#!/usr/bin/env bash
# H26 training run: 228 rows for 6 epochs on physical GPU1.
# Identity prefix inflation collapsed 136 -> 53, ordinary-voice curriculum added.
# 228 rows at 6 epochs is again 90 optimizer steps, so this is comparable to
# H22, H24 and H25 and the difference belongs to the composition change.
set -euo pipefail

REPO="<repo>"
PARENT="<models>/Qwen3.8-27B-Unlocked-BF16"
IMAGE="localhost/naia-persona-train:dev"
DATA="data-private/datasets/naia-v14/train.jsonl"
OUT="data-private/runs/train/naia-v14-qwen38-27b-gpu1-r16-e6-lr5e5-h26"
DATA_SHA="a453dcdc9a52c2f42385e778287aa4065cb91074dc37d065cea92fee7abd7620"
GPU_UUID="GPU-d584beef-b086-bdff-b43c-c31a1b56a611"

cd "$REPO"

[ -f "$DATA" ] || { echo "FATAL: dataset missing: $DATA" >&2; exit 2; }
[ -f "data-private/datasets/naia-v14/manifest.json" ] || { echo "FATAL: H26 dataset manifest missing; build before allocating the GPU" >&2; exit 2; }
...
LOG="$(mktemp -t naia-h26-train-XXXXXX.log)"
...
run_container() {
  podman run --rm --replace \
    ...
    python3 scripts/train_lora.py \
      ...
      --output "$OUT" \
      --profile h26 \
      --epochs 6 --learning-rate 5e-5 --max-length 256 \
...
set +e
{
  echo "=== H26 preflight $(date -Is) ==="
  run_container preflight --preflight-only &&
  echo "=== H26 training $(date -Is) ===" &&
  run_container train &&
  echo "=== H26 done $(date -Is) ==="
} 2>&1 | tee "$LOG"
status="${PIPESTATUS[0]}"
