#!/usr/bin/env bash
# H24 training run: the H23 rows, unchanged, for 6 epochs on physical GPU1.
# Epoch count is the only difference from H23. This restores H22's ~90 optimizer
# steps so the persona and boundary comparison is no longer confounded by volume.
set -euo pipefail
...
OUT="data-private/runs/train/naia-v12-qwen38-27b-gpu1-r16-e6-lr5e5-h24"
...
[ -f "data-private/datasets/naia-v12/freeze-manifest-h24.json" ] || { echo "FATAL: H24 freeze manifest missing; freeze before allocating the GPU" >&2; exit 2; }
...
LOG="$(mktemp -t naia-h24-train-XXXXXX.log)"

run_container() {
  podman run --rm \
    --name "naia-persona-h24-$1" \
...
set +e
{
  echo "=== H24 preflight $(date -Is) ==="
  run_container preflight --preflight-only &&
  echo "=== H24 training $(date -Is) ===" &&
  run_container train &&
  echo "=== H24 done $(date -Is) ==="
} 2>&1 | tee "$LOG"
status="${PIPESTATUS[0]}"
